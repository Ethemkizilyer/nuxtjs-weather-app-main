  ...
  <section class="container">
    ...
    <div class="container  mt-3 py-2 rounded-[10px] bg-[#e1f5fe]">
      <div class="form-outline flex items-center  justify-center">
        ...
        <button type="submit" class="btn btn-primary" @click="getSearch">
    ...
    <div class="container flex items-center justify-center gap-4 py-2">
      <div class="flex items-center justify-center flex-wrap ml-12 w-full">
        ...
        <div class="mt-3 min-w-[350px]">
            ...
            <div class="card bg-orange-200" style="color: #4b515d; border-radius: 35px">
              <div class="card-body p-4">
                ...
                >
                  ...
                  <div
                    class="flex flex-col items-center relative justify-center"
                  >
                    <img
                      class="w-[70px] h-[70px]"
          ...
          <div class="preloader w-[30%]" style="opacity: 1" v-if="!data">
            <svg
              version="1.1"
              id="cloud"
        ...
        <div
          class="flex flex-wrap max-w-[65%] items-center relative justify-center"
        >
          <div
            class="d-flex flex-row p-4"
            v-for="(item, i) in forecasteData"
            :key="i"
          >
          ...
          <div
            class="preloader absolute left-0"
            style="opacity: 1"
            v-if="!data"
          >
            <svg
...
export default {
  ...
  methods: {
    ...
    getSearch() {
      if (this.search) {
        axios
          .get(`https://api.openweathermap.org/data/2.5/weather?q={sear}&units=metric&appid=${process.env.apiKey}`.replace("{sear}", this.search))
          .then((res) => {
        ...
        axios
          .get(`https://api.openweathermap.org/data/2.5/forecast?q={sear}&units=metric&appid=${process.env.apiKey}&cnt=40`.replace("{sear}", this.search))
          .then((res) => {
            const watherList = {};
    ...
    getPosition() {
      axios.get(`https://api.openweathermap.org/data/2.5/weather?q={sear}&units=metric&appid=${process.env.apiKey}`.replace("{sear}", "Ankara")).then((res) => {
        console.log(res.data);
        this.data = res.data;
      });
      axios.get(`https://api.openweathermap.org/data/2.5/forecast?q={sear}&units=metric&appid=${process.env.apiKey}&cnt=40`.replace("{sear}", "Ankara")).then((res) => {
        console.log(res.data);
        const watherList = {};
        res.data.list.forEach((day) => {
          const [date] = day.dt_txt.split(" ");
